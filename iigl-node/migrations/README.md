...
| `038-official-email.sql` | **applied** | `users.official_email` — the franchise's own address, beside the owner's `email` that the account signs in with and a password reset goes to. One box was asked for both, so whichever was typed the other was lost. Additive and nullable; nothing existing changes. |
| `039-attribute-masters.sql` | **applied** | `attribute_masters` and `attribute_master_values`: the library the Add Value form is filled from. A master is an attribute name under a category with the values it normally takes; what is picked from it becomes ordinary `attribute_values` rows and the two are unrelated afterwards. Two new tables, nothing existing read or written. |
| `040-message-topic.sql` | **applied** | `staff_messages.topic` — which template a request was written from, `leave` or `punch`. Both are requests naming a day and differed only in prose the writer can rewrite, so nothing could answer "is this person on leave today"; the Employee list's badge needs that. NULL on every existing row, which is correct. |
| `041-request-decision.sql` | **applied** | `staff_messages.decision` — `approved` or `declined`. `resolved_at` was the whole of a request's state and the panel could only say "Dealt with", which does not answer the one question the person who asked has. NULL beside a set `resolved_at` is a request closed before there was a decision to record, and still reads as "Dealt with". The reply is not a column: it is an ordinary message written back to whoever asked. |
| `042-message-reply-to.sql` | **applied** | `staff_messages.reply_to` — the request a reply answers. 041 sent the approver's sentence back as an ordinary message, which puts it in their inbox, but nothing tied it to the request, so it arrived as its own top-level row above the thing it refuses. |
| `043-link-existing-replies.sql` | **applied** | Links the replies written between 041 and 042. Identified exactly, not guessed: 041 wrote both sides in one transaction, so the reply's `created_at` is the request's `resolved_at` to the second, the two people are the same pair reversed, and the `about_date` matches — all four together. |
| `044-employment-week-off.sql` | **applied** | `employements.week_off` — the days of the week a posting is off, `0`–`6` Sunday-first, comma separated because a five-day week is two days. On the employment rather than the account: somebody moving to a laboratory that closes on a different day has a new week off, and the old row should keep saying what was true while it ran. Empty on every existing row, meaning no fixed day off. |
| `045-registered-customers.sql` | **applied** | `registered_customers` and `customer_category_discounts`. Until now a registered customer was whoever put a GST number on an order, which cannot hold somebody who has not ordered yet or anything an order does not. One record per mobile per laboratory; a discount per category, `percent` or `per_pc`. Stored, not yet applied to billing — pricing is parity-verified ported behaviour. Additive. |
...
| `054-course-card.sql` | **applied** | `courses.image` (uploads/website), `level` (the badge), `track` (the website filter) and `lessons` (as printed, e.g. "12 Lessons"): the rest of a course's card on the website, beside the name, description and duration it already had. Additive. |
| `054-message-attachment.sql` | **applied** | A message may carry one attached image or PDF. |
| `055-course-title.sql` | **applied** | `courses.title` and `subtitle`: the website card's own heading and the line under it, apart from the course name an enrolment and a certificate carry. Additive. |
...
| `056-course-categories.sql` | **applied** | `courses.track` (one of four fixed placeholders, never filled on any course) renamed to `categories`, a JSON array of strings typed in the panel. |
...
| `057-branch-map.sql` | **applied** | `branches.state`, `blurb`, `lat`, `lon`: the website's branch list line and map pin, so Website Setup › Branches is the whole record rather than half of it living in the site's code. Additive. |
