...
settingsRoutes.post(
  ...
  wrap(async (req, res) => {
    ...
    try {
      ...
      res.json({
        data: {
          ok: false,
          message:
            'That is not a valid connection string. A #, @ or : inside the password has to be ' +
            'written %23, %40 or %3A.',
        },
      });
      return;
    ...
    try {
      ...
      res.json({ data: { ok: false, message: explain(e as SmtpError) } });
    } finally {
      transport.close();
    }
  }),
...
function explain(err: SmtpError): string {
  const raw = err.message ?? 'The connection failed.';

  if (err.code === 'EAUTH' || err.responseCode === 535) {
    return (
      'The server refused the username or password. Google no longer accepts an account ' +
