...
const statusOf = (v: unknown, fallback?: Status): Status => {
  if (v == null || v === '') {
    if (fallback) return fallback;
    throw badRequest('A status is required.');
  }
  const s = String(v);
  if (!(ENQUIRY_STATUS as readonly string[]).includes(s)) {
...
enquiryRoutes.get(
  ...
  wrap(async (req, res) => {
    ...
    const filtered = <T extends { where: any }>(q: T): T => {
      let out: any = q;
      if (kind) out = out.where('kind', '=', kind);
    ...
    const [rows, count] = await Promise.all([
      filtered(db.selectFrom('enquiries').selectAll())
        ...
        .execute(),
      filtered(
        db.selectFrom('enquiries').select(db.fn.countAll().as('n')),
...
enquiryRoutes.get(
  ...
  wrap(async (_req, res) => {
    ...
    const kinds = Object.fromEntries(ENQUIRY_KIND.map((k) => [k, 0])) as Record<string, number>;
    for (const r of byKind) kinds[String(r.kind)] = Number(r.n);

    const statuses = Object.fromEntries(ENQUIRY_STATUS.map((s) => [s, 0])) as Record<string, number>;
    for (const r of byStatus) statuses[String(r.status)] = Number(r.n);
