 ...
 */
async function findEnquiry(type: EnquiryType, id: number) {
  const row =
    type === 'enquiry'
...
export async function followupsFor(type: EnquiryType, id: number) {
...
export async function recordFollowup(
  ...
  if (body.status !== undefined && body.status !== null && body.status !== '') {
    ...
    if (!(STATUSES[type] as readonly string[]).includes(to)) {
  ...
  if (moved) {
    ...
    if (type === 'enquiry') patch.closed_at = to === 'closed' ? new Date() : null;
  }
  if (type === 'enquiry') {
    await db.updateTable('enquiries').set(patch as never).where('id', '=', id).execute();
  } else {
    await db.updateTable('student_enquiries').set(patch as never).where('id', '=', id).execute();
  }
...
export async function followupCounts(type: EnquiryType, ids: number[]) {
  if (ids.length === 0) return new Map<number, { n: number; last_at: Date | null }>();
  ...
  return new Map(
    rows.map((r) => [
      Number(r.enquiry_id),
      { n: Number(r.n), last_at: (r.last_at as Date | null) ?? null },
    ]),
  );
}
