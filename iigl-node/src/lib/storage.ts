...
export type StorageStatus =
  | { ok: true; bucket: string; endpoint: string }
...
export async function checkStorage(): Promise<StorageStatus> {
  if (!r2) return { ok: false, reason: 'not configured' };
  try {
...
export async function putObject(
  ...
  if (!r2) throw new Error('Object storage is not configured (R2_* environment variables).');
  await r2.send(
