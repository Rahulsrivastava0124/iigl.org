  ...
  components: {
    ...
    schemas: {
      ...
      CreateReport: {
        ...
        properties: {
          order_id: {
            ...
            examples: [9612],
            description:
              'The order primary key. Stored in the misnamed reports.order_no column, which holds the order id rather than the order number.',
          },
          ...
          attributes: {
            type: 'array',
            description:
              'Form values for the subcategory. For an attribute with is_opensource set, a value not already in attribute_values creates a new row and the report stores its id.',
            items: {
              type: 'object',
              required: ['attr_id'],
              properties: {
      ...
      Quote: {
        type: 'object',
        description:
          'An order priced from the weight bands. Computed on demand — orders carry no price snapshot, so a quote reflects the rates in force today.',
        properties: {
          order_id: { type: 'integer' },
          order_no: { type: 'string' },
          certificates: {
      ...
      Ledger: {
        type: 'object',
        description:
          'Running account for one user. Only approved transactions move the balance; pending and declined entries appear so the history is complete.',
        properties: {
          entries: {
            type: 'array',
            items: {
  ...
  paths: {
    ...
    '/api/public/verify/{reportNo}': {
      get: {
        ...
        description:
          'The endpoint printed QR codes point at. Returns only what is on the card — never the customer or order behind it.',
        security: [],
        parameters: [
          {
            name: 'reportNo',
    ...
    '/api/orders': {
      ...
      post: {
        ...
        description:
          'Writes the order and its items in one database transaction. The order number is random, so a collision is retried rather than issued twice.',
        requestBody: {
          required: true,
    ...
    '/api/reports': {
      ...
      post: {
        ...
        description:
          'Allocates the next report number for the laboratory and writes the attribute values as a JSON blob. An order item accepts certificates up to its quantity and no more.',
        requestBody: {
          required: true,
    ...
    '/api/reports/{id}': {
      patch: {
        ...
        description:
          'The report number is never reallocated - it is printed on a document already in circulation. Supplying attributes replaces the whole set.',
        parameters: [{ name: 'id', in: 'path', required: true, schema: { type: 'integer' } }],
        requestBody: {
          required: true,
          content: {
    ...
    '/api/transactions': {
      ...
      post: {
        ...
        description:
          'A laboratory remits to the administrator; staff remit to their laboratory. Lands pending until the receiver decides.',
        requestBody: {
          required: true,
          content: {
            'application/json': {
    ...
    '/api/transactions/dues/{orderId}': {
      post: {
        ...
        description:
          'Writes the transaction and moves the order balance in one database transaction. Refuses a collection larger than the outstanding amount.',
