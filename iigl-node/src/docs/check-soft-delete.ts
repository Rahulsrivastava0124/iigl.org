...
const TOUCHES_ORDERS =
  /(selectFrom\(\s*'orders'\s*\)|(?:inner|left|right)Join\(\s*'orders'\s*,)/;
...
function walk(dir: string): string[] {
  return readdirSync(dir).flatMap((entry) => {
    ...
    if (statSync(full).isDirectory()) return entry === 'docs' ? [] : walk(full);
...
for (const file of walk(root)) {
  ...
  if (!TOUCHES_ORDERS.test(source)) continue;

  const lines = source.split('\n');
  lines.forEach((line, i) => {
    if (!TOUCHES_ORDERS.test(line)) return;
    ...
    let start = i;
    while (start > 0 && !/(^|\s)(const|let|return|await|=>)\s|^\s*$/.test(lines[start])) start--;
    let end = i;
    ...
    if (FILTERED.test(statement)) return;
