...
const raw = async (query: string) => {
  const r = await sql.raw<Record<string, any>>(query).execute(db);
  return r.rows;
};
...

{
  ...
  for (const key of ['total_sale', 'total_paid', 'total_dues'] as const) {
    const a = Number(php[key]);
    const b = Number(mine[key]);
    report(
      key.replace('_', ' '),
      Math.abs(a - b) < 0.01,
      `laravel ${money(a)}  ·  api ${money(b)}  ·  diff ${money(b - a)}`,
    );
  }
}
...

{
  ...
  for (const u of users) {
    ...
    const balanceOk =
      Math.abs(Number(php.credit) - Number(php.debit) - ledger.balance) < 0.01;

    if (!(creditOk && debitOk && balanceOk)) {
      mismatched++;
      console.log(
        `  DIFFER #${id} ${u.fullname}: laravel credit ${money(Number(php.credit))} debit ${money(
          Number(php.debit),
        )} · api credit ${money(ledger.credit_total)} debit ${money(ledger.debit_total)}`,
      );
    }
  }
...

{
  ...
  const [pending] = await raw(
    `SELECT COALESCE(SUM(amount),0) AS t FROM transactions WHERE transaction_type='commision' AND status=0`,
  );
  const [approved] = await raw(
    `SELECT COALESCE(SUM(amount),0) AS t FROM transactions WHERE transaction_type='commision' AND status=1`,
  );
...

{
  ...
  for (const [i, r] of reports.entries()) {
    for (const a of expanded[i]) {
      ...
      if (/^\d+$/.test(v) && Number(v) > 0) blanks++;
    }
  }

  report(
    'attribute values resolve to names',
    blanks === 0,
    `${reports.length} certificates, ${attributes} attributes, ${blanks} unresolved`,
  );
...

{
  ...
  const [shape] = await raw(
    `SELECT COUNT(*) AS n FROM reports WHERE report_no NOT REGEXP '^[0-9]{12}$'`,
  );
...

{
  ...
  for (const o of priced) {
    ...
    if (Math.abs(q.total_amount - Number(o.total_amount)) < 0.01) match++;
  }
  ...
  `);
  const [untotalled] = await raw(
    `SELECT COUNT(*) AS n FROM orders WHERE total_amount IS NULL OR total_amount=''`,
  );
...

{
  ...
  report(
    ...
    dupes.length === 0,
    dupes.length ? `${dupes.length} numbers shared: ${dupes.map((d) => d.mobile).join(', ')}` : 'no duplicates',
  );
...

{
  ...
  const [badSlug] = await raw(
    `SELECT COUNT(*) AS n FROM blogs WHERE slug IS NULL OR slug='' UNION ALL SELECT COUNT(*) FROM branches WHERE pageURL IS NULL OR pageURL=''`,
  );
