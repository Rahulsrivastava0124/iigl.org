...
async function applied(c: mysql.Connection): Promise<Map<string, string>> {
  const [rows] = await c.query('SELECT name, checksum FROM schema_migrations');
  return new Map((rows as Array<{ name: string; checksum: string }>).map((r) => [r.name, r.checksum]));
}
...
async function main() {
  ...
  if (changed.length) {
    ...
    for (const f of changed) console.error(`  ${f.name}`);
    console.error(
  ...
  if (baseline) {
    for (const f of pending) {
      ...
      await c.query(
        'INSERT INTO schema_migrations (name, checksum, statements) VALUES (?, ?, ?)',
        [f.name, f.checksum, statements(f.sql).length],
      );
  ...
  if (dropping.length && !allowDrop && !dry) {
    ...
    for (const f of dropping) {
      for (const s of f.drops) console.error(`  ${f.name}: ${s}`);
    }
  ...
  for (const f of runnable) {
    ...
    if (dry) {
      ...
      for (const s of parts) console.log(`    ${s.replace(/\s+/g, ' ').slice(0, 100)}`);
      continue;
    }
    ...
    for (const [i, statement] of parts.entries()) {
      try {
        ...
        console.error(
          `  Statements 1 to ${i} were applied and are still in place. ${f.name} is not\n` +
