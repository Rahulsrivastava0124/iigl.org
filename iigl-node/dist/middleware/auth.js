 ...
 */
export async function empidOf(userId) {
    const row = await db
        .selectFrom('users')
        .select('empid')
        .where('id', '=', userId)
 ...
 */
export async function resolveLabId(userId, roleId) {
    if (roleId === ROLE.LAB)
        return userId;
    const row = await db
...
export const requireRole = (predicate, label) => (req, _res, next) => {
    const user = readSession(req);
    if (!user)
        return next(unauthorized());
    req.user = user;
    if (!predicate(user.roleId))
        return next(forbidden(`Requires ${label} access.`));
    next();
};
 ...
 */
export const requireLabScope = requireRole((r) => r === ROLE.SUPER || (r !== null && r >= ROLE.LAB), 'laboratory or administrator');
/**
 ...
 */
export function assertLabOwnership(user, rowLabId) {
