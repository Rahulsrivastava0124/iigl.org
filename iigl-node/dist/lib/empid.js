...
export const prefixFor = (roleId) => Number(roleId) === 2 ? LAB_PREFIX : STAFF_PREFIX;
/**
 ...
 */
export async function nextEmpid(prefix) {
    const rows = await db
        .selectFrom('users')
        .select('empid')
...
export async function empidTaken(empid, exceptUserId) {
