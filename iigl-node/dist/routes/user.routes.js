...
import { franchiseAgreementHtml, franchiseAgreementPdf, franchiseeFormHtml, franchiseeFormPdf, } from '../services/document.service.js';
...
import { effectivePermissionsFor, invalidatePermissions, isActionType, userPermissionsFor, } from '../services/permission.service.js';
 ...
 */
const isSenior = (roleId) => roleId !== null && (Number(roleId) === ROLE.SUPER || Number(roleId) === ROLE.LAB);
 ...
 */
async function currentEmployment(userId) {
    const row = await db
...
userRoutes.get('/me', requireLabScope, wrap(async (req, res) => {
    const row = await db
        .selectFrom('users')
    ...
    */
    const [accruedBy, settled] = await Promise.all([
        ...
        db
            ...
            .select(({ fn }) => ['send_by', fn.sum('amount').as('total')])
 ...
 */
userRoutes.get('/laboratories/:id/detail', numericId, requireAdmin, wrap(async (req, res) => {
 ...
 */
userRoutes.get('/laboratories/:id/agreement', numericId, requireAdmin, wrap(async (req, res) => {
    const labId = Number(req.params.id);
    const lab = await db
        .selectFrom('users')
        .select(['id', 'empid'])
    ...
    res.setHeader('Content-Disposition', `inline; filename="franchise-agreement-${blank ? 'blank' : (lab.empid ?? labId)}.pdf"`);
    res.send(pdf);
}));
userRoutes.get('/laboratories/:id/registration', numericId, requireAdmin, wrap(async (req, res) => {
    const labId = Number(req.params.id);
    const lab = await db
        .selectFrom('users')
        .select(['id', 'empid'])
    ...
    res.setHeader('Content-Disposition', `inline; filename="franchisee-form-${blank ? 'blank' : (lab.empid ?? labId)}.pdf"`);
    res.send(pdf);
}));
/** Staff of a lab, joined through employements. */
userRoutes.get('/staff', requireLabScope, wrap(async (req, res) => {
...
userRoutes.post('/', requireAdmin, wrap(async (req, res) => {
...
userRoutes.patch('/:id/active', numericId, requireAdmin, wrap(async (req, res) => {
 ...
 */
function documentsPatch(given) {
    if (given === undefined)
        return undefined;
    if (given === null || given === '')
        return null;
    if (!Array.isArray(given))
        throw badRequest('Documents must be a list.');
    if (given.length > 25)
        throw badRequest('A laboratory can hold at most 25 documents.');
    const cleaned = given.map((entry, i) => {
        const at = `Document ${i + 1}`;
        if (!entry || typeof entry !== 'object')
            throw badRequest(`${at} is not a document.`);
        const row = entry;
        const path = String(row.path ?? '').trim();
        const title = String(row.title ?? '').trim();
        if (!path)
            throw badRequest(`${at} has no file.`);
        if (path.length > 255)
            throw badRequest(`${at} has an impossible path.`);
        if (!/^(public\/)?uploads\/[A-Za-z0-9._/-]+$/.test(path) || path.includes('..')) {
            throw badRequest(`${at} does not point at an uploaded file.`);
        }
        if (title.length > 191)
            throw badRequest(`${at} has a title longer than 191 characters.`);
        return {
 ...
 */
userRoutes.patch('/me', requireLabScope, wrap(async (req, res) => {
    ...
    if (req.body?.mobile !== undefined) {
        ...
        if (!mobile)
            throw badRequest('Mobile number cannot be blank.');
    ...
    if (patch.email) {
        const taken = await db
            ...
            .executeTakeFirst();
        if (taken) {
            throw conflict(`That email address is already on ${taken.fullname}'s account. A password reset ` +
    ...
    await db.updateTable('users').set(patch).where('id', '=', req.user.id).execute();
    const row = await db
        .selectFrom('users')
        .select(PUBLIC_COLUMNS)
        .where('id', '=', req.user.id)
 ...
 * employment was ended.
 */
userRoutes.get('/:id', numericId, requireAdmin, wrap(async (req, res) => {
 ...
 */
userRoutes.patch('/:id', numericId, requireAdmin, wrap(async (req, res) => {
    const id = Number(req.params.id);
    const row = await db
        .selectFrom('users')
        .select(['id', 'empid'])
    ...
    if (req.body?.mobile !== undefined) {
        ...
        const clash = await db
            ...
            .executeTakeFirst();
        if (clash) {
            throw conflict(`Account ${clash.id} already uses that mobile number. Two accounts sharing a number is what locks people out of sign-in.`);
        }
        patch.mobile = mobile;
    }
    ...
    if (req.body?.commision !== undefined)
        patch.commision = Number(req.body.commision);
    ...
    if (req.body?.empid !== undefined) {
        ...
        if (!empid)
            throw badRequest('Employee ID cannot be blank.');
        ...
        if (empid !== row.empid && row.empid) {
            ...
            if (posting || staff) {
                throw conflict(`Employments still point at "${row.empid}". Changing it would leave those people working for nobody — move them first.`);
            }
        }
        patch.empid = empid;
    }
    if (Object.keys(patch).length === 1)
        throw badRequest('Nothing to update.');
    await db.updateTable('users').set(patch).where('id', '=', id).execute();
    res.json({ ok: true });
}));
/**
 * Delete an account.
 ...
 */
userRoutes.delete('/:id', numericId, requireAdmin, wrap(async (req, res) => {
    const id = Number(req.params.id);
    if (id === req.user.id)
    ...
    const count = async (q) => Number((await q)?.n ?? 0);
    ...
    if (held.length > 0) {
        throw conflict(`${row.fullname} still has ${held.join(', ')}. Deactivate the account instead — deleting it would leave those records belonging to nobody.`);
    }
...
userRoutes.post('/:id/password', numericId, requireAdmin, wrap(async (req, res) => {
 ...
 */
async function setEmployer(userId, parentEmpid, exec = db) {
    await exec
        .updateTable('users')
 ...
 */
async function employ(userId, employerId, details = {}, exec = db) {
    ...
    if (!user)
        throw notFound('Account not found.');
    if (!employer)
        throw badRequest('That employer does not exist.');
    ...
    if (!employer.empid) {
        throw badRequest('That employer has no employee ID. Give it one before employing anybody there — an employment records the employer by their empid.');
    }
 ...
 * employed.
 */
userRoutes.post('/:id/employment', numericId, requireAdmin, wrap(async (req, res) => {
 ...
 */
userRoutes.patch('/:id/employment', numericId, requireAdmin, wrap(async (req, res) => {
    const row = await db
        .selectFrom('employements')
        .select('id')
        ...
        .executeTakeFirst();
    if (!row)
        throw notFound('This person is not currently employed anywhere.');
    const patch = { updated_at: new Date() };
    ...
    if (req.body?.remark !== undefined)
        patch.remark = String(req.body.remark ?? '');
    if (Object.keys(patch).length === 1)
        throw badRequest('Nothing to update.');
    await db
...
userRoutes.post('/:id/employment/end', numericId, requireAdmin, wrap(async (req, res) => {
    const row = await db
        .selectFrom('employements')
        .select('id')
        .where('user_id', '=', Number(req.params.id))
 ...
 */
async function assertMayGrant(user, targetId) {
 ...
 */
userRoutes.get('/me/permissions', requireLabScope, wrap(async (req, res) => {
    res.json({ data: await effectivePermissionsFor(req.user) });
}));
/** What one person has been granted individually, one row per action. */
userRoutes.get('/:id/permissions', numericParams('id'), requireLabScope, wrap(async (req, res) => {
 ...
 */
userRoutes.put('/:id/permissions', numericParams('id'), requireLabScope, wrap(async (req, res) => {
    const userId = Number(req.params.id);
    await assertMayGrant(req.user, userId);
    const action = String(req.body?.action_type ?? '');
    if (!(await isActionType(action)))
        throw badRequest(`${action} is not a permission.`);
    const set = {
...
userRoutes.delete('/:id/permissions/:action', numericParams('id'), requireLabScope, wrap(async (req, res) => {
    const userId = Number(req.params.id);
    await assertMayGrant(req.user, userId);
