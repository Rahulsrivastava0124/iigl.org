...
function amount(v, field) {
    ...
    if (!Number.isFinite(n) || n < 0)
        throw badRequest(`${field} must be a number, and not negative.`);
    return n;
}
...
function limit(v, field) {
    ...
    if (!Number.isInteger(n) || n < 1)
        throw badRequest(`${field} must be a whole number above zero.`);
    return n;
}
const date = (v, field) => {
    const s = text(v);
...
const codeOf = (v, field = 'Code') => requireText(v, field).toUpperCase().replace(/\s+/g, '');
/**
 ...
 */
export function couponDiscount(fee, coupon) {
 ...
 * does not.
 */
async function refusal(c, e) {
    if (!c.is_active)
        return `${c.code} has been switched off.`;
    const from = c.valid_from ? String(c.valid_from).slice(0, 10) : null;
    const to = c.valid_to ? String(c.valid_to).slice(0, 10) : null;
    const now = today();
    if (from && now < from)
        return `${c.code} is not valid until ${from}.`;
    if (to && now > to)
        return `${c.code} expired on ${to}.`;
    ...
    if (min > 0 && fee < min)
        return `${c.code} needs a course fee of at least ${min}.`;
...
// ------------------------------------------------------------------ the list
couponRoutes.get('/', wrap(async (req, res) => {
    const p = readPage(req);
    const term = String(req.query.q ?? '').trim();
    const base = () => {
        let q = db.selectFrom('discount_coupons');
        if (req.query.course_id)
            q = q.where('course_id', '=', Number(req.query.course_id));
        if (req.query.active === '1')
            q = q.where('is_active', '=', 1);
        if (req.query.active === '0')
            q = q.where('is_active', '=', 0);
        if (term) {
            q = q.where((eb) => eb.or([eb('code', 'like', `%${term}%`), eb('title', 'like', `%${term}%`)]));
        }
        return q;
    };
...
couponRoutes.post('/', wrap(async (req, res) => {
    ...
    if (value <= 0)
        throw badRequest('A coupon that takes off nothing is not a coupon.');
    if (type === 'percent' && value > 100)
        throw badRequest('A percentage cannot exceed 100.');
    ...
    if (courseId !== null) {
        const course = await db
            ...
            .executeTakeFirst();
        if (!course)
            throw badRequest('That course does not exist.');
    }
    const clash = await db
        ...
        .executeTakeFirst();
    if (clash)
        throw conflict(`${code} is already a coupon.`);
    const result = await db
        ...
        max_discount: req.body?.max_discount == null || req.body?.max_discount === ''
            ? null
...
couponRoutes.get('/:id', numericId, wrap(async (req, res) => {
    res.json({ data: await coupon(Number(req.params.id)) });
}));
couponRoutes.patch('/:id', numericId, wrap(async (req, res) => {
    ...
    if (req.body?.title !== undefined)
        patch.title = text(req.body.title);
    if (req.body?.description !== undefined)
        patch.description = text(req.body.description);
    ...
    const from = (patch.valid_from ?? row.valid_from);
    const to = (patch.valid_to ?? row.valid_to);
 ...
 */
couponRoutes.patch('/:id/active', numericId, wrap(async (req, res) => {
    const row = await coupon(Number(req.params.id));
    await db
        .updateTable('discount_coupons')
...
couponRoutes.delete('/:id', numericId, wrap(async (req, res) => {
    const row = await coupon(Number(req.params.id));
    const used = await db
        .selectFrom('coupon_redemptions')
        .select(({ fn }) => fn.countAll().as('n'))
    ...
    if (times > 0) {
        throw conflict(`${row.code} has been used ${times} time${times === 1 ? '' : 's'}. Switch it off instead — deleting it would take the record of those discounts with it.`);
    }
...
async function byCode(code) {
    const row = await db
        ...
        .executeTakeFirst();
    if (!row)
        throw notFound(`${code} is not a coupon.`);
    return row;
}
/**
 ...
 * gap.
 */
couponRoutes.post('/redeem', wrap(async (req, res) => {
    const c = await byCode(codeOf(req.body?.code));
    const e = await enrolment(req.body?.enrolment_id);
    const result = await db.transaction().execute(async (trx) => {
        ...
        const row = (await trx
            ...
            .forUpdate()
            .executeTakeFirstOrThrow());
        const no = await refusal(fresh, row);
        if (no)
            throw conflict(no);
        ...
        if (finalFee < Number(row.fee_paid)) {
            throw conflict(`${fresh.code} would put the fee below the ${Number(row.fee_paid)} already paid.`);
        }
        await trx
...
couponRoutes.get('/:id/redemptions', numericId, wrap(async (req, res) => {
    const row = await coupon(Number(req.params.id));
    const p = readPage(req);
    const [rows, count] = await Promise.all([
        db
