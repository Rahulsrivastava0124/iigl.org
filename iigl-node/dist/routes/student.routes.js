...
export const REGISTRATION_STATUS = ['pending', 'registered', 'active'];
export const COURSE_STATUS = ['upcoming', 'ongoing', 'completed'];
...
function oneOf(allowed, v, fallback) {
    if (v == null || v === '') {
        if (fallback)
            return fallback;
    ...
    if (!allowed.includes(s))
        throw badRequest(`Unknown value. Expected one of: ${allowed.join(', ')}.`);
    return s;
}
/** A `like` filter over a few columns, shared by every list here. */
function search(q, term, columns) {
    if (!term)
        return q;
    const like = `%${term}%`;
    return q.where((eb) => eb.or(columns.map((c) => eb(c, 'like', like))));
}
// ===========================================================================
// 1. Enquiry
// ===========================================================================
studentRoutes.get('/enquiries', wrap(async (req, res) => {
...
studentRoutes.patch('/enquiries/:id', numericId, wrap(async (req, res) => {
 ...
 */
studentRoutes.post('/enquiries/:id/convert', numericId, wrap(async (req, res) => {
    const enquiryId = Number(req.params.id);
    const b = req.body ?? {};
...
studentRoutes.delete('/enquiries/:id', numericId, wrap(async (req, res) => {
    const result = await db
 ...
 */
async function nextRegistrationNo(trx) {
...
studentRoutes.get('/', wrap(async (req, res) => {
    const p = readPage(req);
...
studentRoutes.get('/summary', wrap(async (_req, res) => {
    const [enquiries, registrations, enrolments, fees, certificates] = await Promise.all([
        ...
        db
            ...
            .select(({ fn }) => [
            fn.sum('fee').as('fee'),
            fn.sum('discount_amount').as('discount'),
            fn.sum('final_fee').as('billed'),
            fn.sum('fee_paid').as('collected'),
        ])
            .executeTakeFirstOrThrow(),
        db
...
studentRoutes.get('/:id', numericId, wrap(async (req, res) => {
...
studentRoutes.post('/', wrap(async (req, res) => {
    const b = req.body ?? {};
    const studentId = await db.transaction().execute(async (trx) => {
        const inserted = await trx
            .insertInto('students')
...
studentRoutes.patch('/:id', numericId, wrap(async (req, res) => {
    const studentId = Number(req.params.id);
    const existing = await db
        .selectFrom('students')
        .select('id')
    ...
    if (b.father_name !== undefined)
        patch.father_name = text(b.father_name);
    if (b.dob !== undefined)
        patch.dob = date(b.dob);
    if (b.gender !== undefined)
        patch.gender = text(b.gender);
    if (b.mobile !== undefined)
        patch.mobile = requireText(b.mobile, 'Mobile number');
    if (b.alt_mobile !== undefined)
        patch.alt_mobile = text(b.alt_mobile);
    if (b.email !== undefined)
        patch.email = text(b.email);
    if (b.address !== undefined)
        patch.address = text(b.address);
    if (b.city !== undefined)
        patch.city = text(b.city);
    if (b.state !== undefined)
        patch.state = text(b.state);
    if (b.pincode !== undefined)
        patch.pincode = text(b.pincode);
    if (b.photo !== undefined)
        patch.photo = text(b.photo);
    if (b.id_proof !== undefined)
        patch.id_proof = text(b.id_proof);
    if (b.qualification_doc !== undefined)
        patch.qualification_doc = text(b.qualification_doc);
    if (b.registration_date !== undefined)
        patch.registration_date = date(b.registration_date);
    if (b.course_id !== undefined)
        patch.course_id = id(b.course_id);
    if (b.status !== undefined)
        patch.status = oneOf(REGISTRATION_STATUS, b.status);
...
studentRoutes.delete('/:id', numericId, wrap(async (req, res) => {
    const studentId = Number(req.params.id);
