...
async function nextCertificateNo(trx) {
...
studentCertificateRoutes.get('/', wrap(async (req, res) => {
    ...
    const build = (base) => {
        let q = base;
        if (studentId)
            q = q.where('cert.student_id', '=', studentId);
        if (term) {
            const like = `%${term}%`;
            q = q.where((eb) => eb.or([
 ...
 * completed ones.
 */
studentCertificateRoutes.get('/pending', wrap(async (_req, res) => {
    const rows = await db
...
studentCertificateRoutes.post('/', wrap(async (req, res) => {
    const b = req.body ?? {};
    const enrolmentId = Number(b.student_course_id);
    if (!enrolmentId)
        throw badRequest('An enrolment is required.');
    ...
    const already = await db
        ...
        .executeTakeFirst();
    if (already)
        throw conflict('A certificate has already been issued for this enrolment.');
...
studentCertificateRoutes.patch('/:id', numericId, wrap(async (req, res) => {
    ...
    const b = req.body ?? {};
    const patch = {};
    if (b.issued_on !== undefined)
        patch.issued_on = date(b.issued_on);
    if (b.grade !== undefined)
        patch.grade = text(b.grade);
    if (b.remark !== undefined)
        patch.remark = text(b.remark);
    if (b.file !== undefined)
        patch.file = text(b.file);
...
studentCertificateRoutes.delete('/:id', numericId, wrap(async (req, res) => {
