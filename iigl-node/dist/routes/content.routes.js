...
contentRoutes.post('/blogs', wrap(async (req, res) => {
    ...
    if (clash)
        throw conflict(`An article already uses the address /${slug}.`);
...
contentRoutes.patch('/blogs/:id', numericId, wrap(async (req, res) => {
    ...
    if (req.body?.slug !== undefined) {
        ...
        const clash = await db
            ...
            .executeTakeFirst();
        if (clash)
            throw conflict(`Another article already uses the address /${slug}.`);
        patch.slug = slug;
    }
    if (Object.keys(patch).length === 1)
        throw badRequest('Nothing to update.');
    await db.updateTable('blogs').set(patch).where('id', '=', id).execute();
    res.json({ ok: true });
}));
// ---------------------------------------------------------------- branches
const BRANCH_FIELDS = [
...
contentRoutes.post('/branches', wrap(async (req, res) => {
    ...
    const clash = await db
        ...
        .executeTakeFirst();
    if (clash)
        throw conflict(`A branch page already uses the address /${pageURL}.`);
    const values = {
        city,
        pageURL,
        created_at: new Date(),
    ...
    for (const k of BRANCH_FIELDS)
        if (k !== 'city')
            values[k] = text(req.body?.[k]);
    const result = await db.insertInto('branches').values(values).executeTakeFirst();
    res.status(201).json({ data: { id: Number(result.insertId), pageURL } });
}));
contentRoutes.patch('/branches/:id', numericId, wrap(async (req, res) => {
    const id = Number(req.params.id);
    ...
    if (req.body?.pageURL !== undefined) {
        ...
        const clash = await db
            ...
            .executeTakeFirst();
        if (clash)
            throw conflict(`Another branch page already uses /${pageURL}.`);
        patch.pageURL = pageURL;
    }
    if (Object.keys(patch).length === 1)
        throw badRequest('Nothing to update.');
    await db.updateTable('branches').set(patch).where('id', '=', id).execute();
    res.json({ ok: true });
}));
// ------------------------------------------------------------ report types
...
contentRoutes.post('/report-types', wrap(async (req, res) => {
    const values = {
    ...
    for (const k of REPORT_TYPE_FIELDS)
        if (k !== 'name')
            values[k] = text(req.body?.[k]) ?? '';
    const result = await db.insertInto('reporttypes').values(values).executeTakeFirst();
    res.status(201).json({ data: { id: Number(result.insertId) } });
}));
contentRoutes.patch('/report-types/:id', numericId, wrap(async (req, res) => {
    const id = Number(req.params.id);
    ...
    for (const k of REPORT_TYPE_FIELDS) {
        if (req.body?.[k] !== undefined)
            patch[k] = text(req.body[k]) ?? '';
    }
    if (Object.keys(patch).length === 1)
        throw badRequest('Nothing to update.');
    await db.updateTable('reporttypes').set(patch).where('id', '=', id).execute();
    res.json({ ok: true });
}));
// ----------------------------------------------------------------- banners
contentRoutes.get('/banners', wrap(async (_req, res) => {
    res.json({ data: await db.selectFrom('banners').selectAll().orderBy('id').execute() });
}));
contentRoutes.post('/banners', wrap(async (req, res) => {
    const result = await db
        .insertInto('banners')
        .values({
...
contentRoutes.patch('/banners/:id', numericId, wrap(async (req, res) => {
    const id = Number(req.params.id);
    const row = await db.selectFrom('banners').select('id').where('id', '=', id).executeTakeFirst();
    if (!row)
        throw notFound('Banner not found.');
    const patch = { updated_at: new Date() };
    for (const k of ['path', 'img_type', 'name', 'url']) {
        if (req.body?.[k] !== undefined)
            patch[k] = text(req.body[k]);
    }
    if (req.body?.status !== undefined)
        patch.status = req.body.status ? 1 : 0;
    if (Object.keys(patch).length === 1)
        throw badRequest('Nothing to update.');
    await db.updateTable('banners').set(patch).where('id', '=', id).execute();
    res.json({ ok: true });
}));
contentRoutes.delete('/banners/:id', numericId, wrap(async (req, res) => {
    const id = Number(req.params.id);
    const row = await db.selectFrom('banners').select('id').where('id', '=', id).executeTakeFirst();
    if (!row)
        throw notFound('Banner not found.');
...
contentRoutes.get('/pages', wrap(async (_req, res) => {
    const rows = await db
        .selectFrom('websites')
...
contentRoutes.patch('/pages/:id', numericId, wrap(async (req, res) => {
    const id = Number(req.params.id);
    ...
    if (Object.keys(patch).length === 1)
        throw badRequest('Nothing to update.');
    await db.updateTable('websites').set(patch).where('id', '=', id).execute();
    res.json({ ok: true });
}));
// ------------------------------------------------------------------- roles
contentRoutes.post('/roles', wrap(async (req, res) => {
    ...
    const clash = await db
        ...
        .executeTakeFirst();
    if (clash)
        throw conflict(`A role called ${name} already exists.`);
    const result = await db
        .insertInto('roles')
...
contentRoutes.patch('/roles/:id', numericId, wrap(async (req, res) => {
    const id = Number(req.params.id);
    const name = required(req.body?.role_name, 'Role name');
