...
import { createReport, expandAttributes, updateReport, validateReportInput, validateUpdateReportInput, } from '../services/report.service.js';
import { numericId } from '../middleware/params.js';
export const reportRoutes = Router();
reportRoutes.use(requireLabScope);
reportRoutes.get('/', wrap(async (req, res) => {
...
reportRoutes.get('/:id', numericId, wrap(async (req, res) => {
    const row = await db
        .selectFrom('reports')
        .selectAll()
...
reportRoutes.post('/', wrap(async (req, res) => {
 ...
 */
reportRoutes.patch('/:id', numericId, wrap(async (req, res) => {
    const row = await db
        .selectFrom('reports')
        .select(['id', 'lab_id'])
        .where('id', '=', Number(req.params.id))
