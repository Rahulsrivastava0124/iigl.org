...
async function scope(user) {
    if (user.roleId === ROLE.SUPER)
        return { kind: 'all' };
    if ((await orderVisibility(user)) === 'own')
        return { kind: 'own', id: user.id };
    return { kind: 'lab', id: user.labId };
}
...
customerRoutes.get('/registered', wrap(async (req, res) => {
...
customerRoutes.get('/unregistered', wrap(async (req, res) => {
    const p = readPage(req);
    const search = readSearch(req, ['customer_name', 'mobile', 'email', 'gst']);
 ...
 */
customerRoutes.get('/verifiers', wrap(async (req, res) => {
    const p = readPage(req);
