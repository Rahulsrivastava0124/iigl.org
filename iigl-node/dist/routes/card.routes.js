...
import { renderCardsHtml, renderCardsPdf } from '../services/pdf.service.js';
import { orderDocumentHtml, orderDocumentPdf } from '../services/document.service.js';
...
async function authorisedReports(user, ids) {
    const rows = await db
        .selectFrom('reports')
        .select(['id', 'lab_id'])
        .where('id', 'in', ids)
    ...
    for (const id of ids) {
        ...
        if (labId === undefined)
            throw notFound(`Certificate ${id} not found.`);
        assertLabOwnership(user, labId);
    }
    return ids;
}
function send(res, pdf, filename) {
 ...
 * kind = "data".
 */
cardRoutes.get('/data/:id', numericId, wrap(async (req, res) => {
 ...
 * for visual drift.
 */
cardRoutes.get('/:kind/:id', numericId, wrap(async (req, res) => {
 ...
 */
cardRoutes.post('/:kind', wrap(async (req, res) => {
    const kind = kindFrom(String(req.params.kind));
    ...
    const ids = raw.map((v) => {
        ...
        if (!Number.isInteger(n))
            throw badRequest('Every report id must be a number.');
        return n;
    });
 ...
 */
cardRoutes.get('/order/:kind/:id', numericId, wrap(async (req, res) => {
    ...
    if (req.query.format === 'html') {
        res.type('html').send(await orderDocumentHtml(id, kind));
        return;
    }
    send(res, await orderDocumentPdf(id, kind), `${order.order_no}-${kind}.pdf`);
}));
