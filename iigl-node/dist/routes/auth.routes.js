 ...
 */
async function soleAccountFor(identifier) {
    const rows = await db
        .selectFrom('users')
    ...
    return { why: 'ok', user: withEmail[0] };
}
/**
...
authRoutes.post('/forgot-password', wrap(async (req, res) => {
    ...
    const identifier = String(req.body?.identifier ?? req.body?.email ?? '')
        ...
        .toLowerCase();
    if (!identifier)
        throw badRequest('Enter your mobile number or the email on your account.');
    /*
    ...
    */
    ...
    if (found.why === 'no-email') {
        throw badRequest('That account has no email address on it, so there is nowhere to send the link. ' +
            'Ask an administrator to set a new password for you.');
    }
    if (found.why === 'ambiguous') {
        throw badRequest('More than one active account uses that. Try your mobile number instead, or ask ' +
            'an administrator.');
    }
    ...
    res.json({
        ...
        message: `A link to choose a new password is on its way to ${maskEmail(email)}. ` +
...
authRoutes.post('/reset-password', wrap(async (req, res) => {
    ...
    */
    ...
    if (row.user_id) {
        ...
        if (found.why !== 'ok') {
            throw badRequest('This address no longer identifies a single active account, so it cannot say which ' +
