...
import { actionTypes, invalidatePermissions, isActionType, permissionsFor, } from '../services/permission.service.js';
/**
...
const ownerFor = (user) => user.roleId === ROLE.SUPER ? null : Number(user.labId ?? user.id);
/** A role this person may see. Head office sees every one. */
async function visible(user, roleId) {
    const role = await db
        .selectFrom('roles')
        .selectAll()
        .where('id', '=', roleId)
 ...
 */
async function grantable(user, roleId) {
 ...
 */
const ESSENTIAL = [ROLE.SUPER, ROLE.ADMIN, ROLE.TEAM];
/**
 ...
 */
async function renameable(user, roleId) {
    const role = await visible(user, roleId);
    if (user.roleId === ROLE.SUPER)
        return role;
    const mine = ownerFor(user);
 ...
 */
async function deletable(user, roleId) {
    const role = await renameable(user, roleId);
    if (ESSENTIAL.includes(Number(role.id))) {
        throw forbidden(`${role.role_name} is one of the three roles the system is built on — super admin, admin and team. It can be renamed, but not deleted.`);
    }
    return role;
}
// ------------------------------------------------------------------- roles
roleRoutes.get('/', wrap(async (req, res) => {
    let q = db
        .selectFrom('roles')
...
roleRoutes.post('/', wrap(async (req, res) => {
    ...
    const clash = await db
        ...
        .where((eb) => owner === null ? eb('owner_id', 'is', null) : eb.or([eb('owner_id', 'is', null), eb('owner_id', '=', owner)]))
        .executeTakeFirst();
    if (clash)
...
roleRoutes.patch('/:id', numericId, wrap(async (req, res) => {
    const role = await renameable(req.user, Number(req.params.id));
    const patch = {};
    if (req.body?.name !== undefined)
        patch.role_name = requireText(req.body.name, 'Role name');
...
roleRoutes.delete('/:id', numericId, wrap(async (req, res) => {
    ...
    if (holders > 0) {
        throw conflict(`${holders} ${holders === 1 ? 'person holds' : 'people hold'} this role. Move ${holders === 1 ? 'them' : 'them all'} to another role first.`);
    }
...
// ------------------------------------------------------------------- users
roleRoutes.get('/:id/users', numericId, wrap(async (req, res) => {
...
// ------------------------------------------------------------- permissions
roleRoutes.get('/actions', wrap(async (_req, res) => {
 ...
 */
roleRoutes.post('/actions', requireAdmin, wrap(async (req, res) => {
    const name = requireText(req.body?.name, 'Name')
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, '_')
        .replace(/^_|_$/g, '');
    if (!name)
        throw badRequest('A name needs at least one letter or number.');
    if (await isActionType(name))
        throw conflict(`${name} is already on the list.`);
    await db
...
roleRoutes.get('/:id/permissions', numericId, wrap(async (req, res) => {
    const role = await visible(req.user, Number(req.params.id));
    res.json({ data: await permissionsFor(Number(role.id)) });
}));
/** Replace the flags for one action on one role. */
roleRoutes.put('/:id/permissions', numericId, wrap(async (req, res) => {
