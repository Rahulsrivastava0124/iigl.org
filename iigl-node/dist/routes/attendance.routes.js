 ...
 */
const isOpen = (clockOut) => !clockOut || clockOut === '00:00:00';
...
attendanceRoutes.get('/today', wrap(async (req, res) => {
...
attendanceRoutes.post('/clock-in', wrap(async (req, res) => {
...
attendanceRoutes.post('/clock-out', wrap(async (req, res) => {
    const row = await todayFor(req.user.id);
    if (!row)
        throw badRequest('You have not clocked in today.');
    if (!isOpen(row.clockOut))
...
attendanceRoutes.post('/break', wrap(async (req, res) => {
    ...
    await db
        .updateTable('attendances')
        .set(starting
        ? { break_begin: stamp(), updated_at: new Date() }
        : { break_end: stamp(), updated_at: new Date() })
        .where('id', '=', Number(row.id))
        .execute();
 ...
 */
attendanceRoutes.get('/', wrap(async (req, res) => {
    ...
    if (req.query.emp_id) {
        ...
        if (req.user.roleId === ROLE.SUPER) {
            ...
            const employed = mine
                ...
                : undefined;
            if (!employed)
                throw badRequest('That person does not work at your laboratory.');
            target = requested;
        }
    ...
    const range = (value) => {
        ...
        if (!/^\d{4}-\d{2}-\d{2}$/.test(v))
            throw badRequest('Dates are YYYY-MM-DD.');
        return v;
    };
    ...
    const scoped = () => {
        ...
        if (from)
            q = q.where('date', '>=', new Date(`${from}T00:00:00`));
        if (to)
            q = q.where('date', '<=', new Date(`${to}T00:00:00`));
        return q;
    };
