 ...
 */
export function discountOf(fee, type, value) {
    if (!type || value <= 0)
        return 0;
...
// ============================================================== the catalogue
courseRoutes.get('/', wrap(async (req, res) => {
...
courseRoutes.post('/', wrap(async (req, res) => {
    ...
    const clash = await db
        ...
        .executeTakeFirst();
    if (clash)
        throw conflict(`A course called ${name} already exists.`);
...
courseRoutes.patch('/:id', numericId, wrap(async (req, res) => {
    ...
    const patch = {};
    if (b.name !== undefined)
        patch.name = requireText(b.name, 'Course name');
    if (b.code !== undefined)
        patch.code = text(b.code);
    ...
    if (b.is_active !== undefined)
        patch.is_active = b.is_active ? 1 : 0;
...
courseRoutes.delete('/:id', numericId, wrap(async (req, res) => {
    const courseId = Number(req.params.id);
...
courseRoutes.get('/enrolments', wrap(async (req, res) => {
    ...
    const build = (base) => {
        ...
        if (studentId)
            q = q.where('sc.student_id', '=', studentId);
        if (discounted)
            q = q.where('sc.discount_amount', '>', '0');
        if (term) {
            const like = `%${term}%`;
            q = q.where((eb) => eb.or([
                eb('s.name', 'like', like),
    ...
    const [rows, count] = await Promise.all([
        ...
        build(db
            ...
            .leftJoin('students as s', 's.id', 'sc.student_id')
            .leftJoin('courses as c', 'c.id', 'sc.course_id')
            .select(db.fn.countAll().as('n'))).executeTakeFirstOrThrow(),
    ]);
    res.json(paged(rows, Number(count.n), p));
}));
courseRoutes.post('/enrolments', wrap(async (req, res) => {
    const b = req.body ?? {};
    const studentId = Number(b.student_id);
    const courseId = Number(b.course_id);
    if (!studentId || !courseId)
        throw badRequest('A student and a course are required.');
    ...
    const already = await db
        ...
        .executeTakeFirst();
    if (already)
        throw conflict('That student is already on this course.');
...
courseRoutes.patch('/enrolments/:id', numericId, wrap(async (req, res) => {
    ...
    if (b.batch !== undefined)
        patch.batch = text(b.batch);
    if (b.start_date !== undefined)
        patch.start_date = date(b.start_date);
    if (b.end_date !== undefined)
        patch.end_date = date(b.end_date);
    if (b.remark !== undefined)
        patch.remark = text(b.remark);
    if (b.result !== undefined)
        patch.result = text(b.result);
    if (b.fee_paid !== undefined)
        patch.fee_paid = String(money(b.fee_paid, 'Fee paid'));
    if (b.fee !== undefined) {
 ...
 */
courseRoutes.patch('/enrolments/:id/discount', numericId, wrap(async (req, res) => {
    const enrolmentId = Number(req.params.id);
    const existing = await db
        .selectFrom('student_courses')
        .selectAll()
    ...
    if (type === 'fixed' && value > fee)
        throw badRequest('The discount is more than the course fee.');
...
courseRoutes.post('/enrolments/:id/payment', numericId, wrap(async (req, res) => {
    const enrolmentId = Number(req.params.id);
