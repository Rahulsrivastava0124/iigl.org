...
import { ledgerFor, sendCommission, validateCommissionInput, } from '../services/commission.service.js';
...
transactionRoutes.get('/', wrap(async (req, res) => {
    ...
    const ids = [...new Set(rows.flatMap((r) => [Number(r.send_by), Number(r.received_by)]))].filter((id) => id > 0);
 ...
 */
transactionRoutes.post('/', wrap(async (req, res) => {
    ...
    if (!pay_mode)
        throw badRequest('Select a payment mode.');
    const receivedBy = req.user.roleId === ROLE.LAB ? ROLE.SUPER : req.user.labId;
    if (receivedBy === null)
        throw badRequest('Your account is not linked to a laboratory.');
 ...
 */
transactionRoutes.post('/:id/status', numericId, wrap(async (req, res) => {
 ...
 */
transactionRoutes.post('/dues/:orderId', numericParams('orderId'), wrap(async (req, res) => {
    ...
    await db.transaction().execute(async (trx) => {
        ...
        if (value > dues)
            throw badRequest(`Collection exceeds the outstanding balance of ${dues}.`);
        await trx
            .insertInto('transactions')
            .values({
            amount: String(value),
...
transactionRoutes.get('/wallet', wrap(async (req, res) => {
 ...
 */
transactionRoutes.post('/commission', wrap(async (req, res) => {
...
transactionRoutes.get('/ledger', wrap(async (req, res) => {
    const target = req.user.roleId === ROLE.SUPER && req.query.user_id
