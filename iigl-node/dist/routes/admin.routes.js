...
const positive = (v, field) => {
    ...
    if (!Number.isFinite(n) || n < 0)
        throw badRequest(`${field} must be a number of zero or more.`);
    return n;
};
// ---------------------------------------------------------------- categories
adminRoutes.post('/categories', wrap(async (req, res) => {
    ...
    const clash = await db
        ...
        .executeTakeFirst();
    if (clash)
        throw conflict(`A category called ${name} already exists.`);
...
adminRoutes.patch('/categories/:id', numericId, wrap(async (req, res) => {
    ...
    if (req.body?.name !== undefined)
        patch.name = requireText(req.body.name, 'Name');
    if (req.body?.unit !== undefined)
        patch.unit = String(positive(req.body.unit, 'Weight unit'));
    for (const key of ['description', 'short_description', 'banner', 'icon']) {
        if (req.body?.[key] !== undefined)
            patch[key] = text(req.body[key]);
    }
...
// ------------------------------------------------------------- subcategories
adminRoutes.post('/subcategories', wrap(async (req, res) => {
    const name = requireText(req.body?.name, 'Name');
    ...
    const clash = await db
        ...
        .executeTakeFirst();
    if (clash)
        throw conflict(`A subcategory called ${name} already exists.`);
    const result = await db
...
adminRoutes.patch('/subcategories/:id', numericId, wrap(async (req, res) => {
    const id = Number(req.params.id);
    ...
    if (req.body?.category_id !== undefined)
        patch.category_id = positive(req.body.category_id, 'Category');
    for (const key of ['description', 'banner', 'icon']) {
        if (req.body?.[key] !== undefined)
            patch[key] = text(req.body[key]) ?? '';
    }
    if (Object.keys(patch).length === 1)
        throw badRequest('Nothing to update.');
    await db.updateTable('subcategories').set(patch).where('id', '=', id).execute();
    res.json({ ok: true });
}));
// ---------------------------------------------------------------- attributes
adminRoutes.post('/attributes', wrap(async (req, res) => {
    const result = await db
...
adminRoutes.patch('/attributes/:id', numericId, wrap(async (req, res) => {
    const id = Number(req.params.id);
    const row = await db.selectFrom('attributes').select('id').where('id', '=', id).executeTakeFirst();
    if (!row)
        throw notFound('Attribute not found.');
    const patch = { updated_at: new Date() };
    if (req.body?.attr_name !== undefined)
        patch.attr_name = requireText(req.body.attr_name, 'Attribute name');
    if (req.body?.order_no !== undefined)
        patch.order_no = Number(req.body.order_no) || 0;
    ...
    for (const key of [
        ...
        if (req.body?.[key] !== undefined)
            patch[key] = flag(req.body[key]);
    }
    if (Object.keys(patch).length === 1)
        throw badRequest('Nothing to update.');
    await db.updateTable('attributes').set(patch).where('id', '=', id).execute();
    res.json({ ok: true });
}));
/**
 ...
 * with a blank field.
 */
adminRoutes.delete('/attributes/:id', numericId, wrap(async (req, res) => {
    const id = Number(req.params.id);
    const row = await db.selectFrom('attributes').select('id').where('id', '=', id).executeTakeFirst();
    if (!row)
        throw notFound('Attribute not found.');
...
// ---------------------------------------------------------- attribute values
adminRoutes.post('/attribute-values', wrap(async (req, res) => {
    ...
    const attr = await db
        ...
        .executeTakeFirst();
    if (!attr)
        throw badRequest('That attribute does not exist.');
    const clash = await db
        ...
        .executeTakeFirst();
    if (clash)
        throw conflict(`${valueName} is already a value for this attribute.`);
    const result = await db
...
adminRoutes.patch('/attribute-values/:id', numericId, wrap(async (req, res) => {
    const id = Number(req.params.id);
    const row = await db.selectFrom('attribute_values').select('id').where('id', '=', id).executeTakeFirst();
    if (!row)
        throw notFound('Attribute value not found.');
    const patch = { updated_at: new Date() };
    if (req.body?.value_name !== undefined)
        patch.value_name = requireText(req.body.value_name, 'Value');
    for (const key of ['description', 'icon']) {
        if (req.body?.[key] !== undefined)
            patch[key] = text(req.body[key]);
    }
    ...
    if (Object.keys(patch).length === 1)
        throw badRequest('Nothing to update.');
    await db.updateTable('attribute_values').set(patch).where('id', '=', id).execute();
    res.json({ ok: true });
}));
adminRoutes.delete('/attribute-values/:id', numericId, wrap(async (req, res) => {
    const id = Number(req.params.id);
    const row = await db.selectFrom('attribute_values').select('id').where('id', '=', id).executeTakeFirst();
    if (!row)
        throw notFound('Attribute value not found.');
...
adminRoutes.get('/prices', wrap(async (req, res) => {
    ...
    if (req.query.lab_id === 'standard')
        q = q.where('lab_id', 'is', null);
    else if (req.query.lab_id)
        q = q.where('lab_id', '=', String(Number(req.query.lab_id)));
    if (req.query.category_id)
        q = q.where('category_id', '=', String(Number(req.query.category_id)));
 ...
 * on the weight.
 */
async function assertNoOverlap(categoryId, labId, minWt, maxWt, excludeId) {
    let q = db
        .selectFrom('prices')
    ...
    if (clash) {
        throw conflict(`That range overlaps an existing band of ${clash.min_wt} to ${clash.max_wt}. Adjust the range or edit that band.`);
    }
}
adminRoutes.post('/prices', wrap(async (req, res) => {
    ...
    if (maxWt <= minWt)
        throw badRequest('Maximum weight must be greater than the minimum.');
...
adminRoutes.patch('/prices/:id', numericId, wrap(async (req, res) => {
    const id = Number(req.params.id);
    const row = await db.selectFrom('prices').selectAll().where('id', '=', id).executeTakeFirst();
    if (!row)
        throw notFound('Price band not found.');
    ...
    const patch = { updated_at: new Date(), min_wt: minWt, max_wt: maxWt };
    if (req.body?.rate !== undefined)
        patch.rate = String(req.body.rate);
    if (req.body?.smart_price !== undefined)
...
adminRoutes.delete('/prices/:id', numericId, wrap(async (req, res) => {
    const id = Number(req.params.id);
...
adminRoutes.patch('/laboratories/:id/commission', numericId, wrap(async (req, res) => {
    const id = Number(req.params.id);
    ...
    if (Number(row.role_id) !== 2)
        throw badRequest('That account is not a laboratory.');
    await db
        .updateTable('users')
