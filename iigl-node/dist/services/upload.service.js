 ...
 * files would produce.
 */
export function objectKey(stored) {
    return stored.replace(/^\/?public\//, '').replace(/^\/+/, '');
}
/** Where a stored path can be read from, if anywhere. */
export function readUrlFor(stored) {
    return publicUrlFor(objectKey(stored));
}
const extensionFor = (file) => EXTENSIONS[file.mimetype] ?? path.extname(file.originalname).toLowerCase() ?? '';
/**
 ...
 */
export async function store(bucket, file) {
