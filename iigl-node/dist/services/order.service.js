...
export function ddmmyyyy(d = new Date()) {
 ...
 */
export const live = (q) => q.where('deleted_at', 'is', null);
export const liveJoined = (q) => q.where('orders.deleted_at', 'is', null);
export function validateOrderInput(body) {
    ...
    if (!items.length)
        throw badRequest('Add at least one item to the order.');
    const parsed = items.map((raw, i) => {
        const it = (raw ?? {});
        const qty = Number(it.qty);
        if (!it.category_id)
            throw badRequest(`Item ${i + 1} is missing a category.`);
        if (!Number.isInteger(qty) || qty < 1)
            throw badRequest(`Item ${i + 1} needs a quantity of at least 1.`);
...
export async function createOrder(user, input) {
    ...
    return db.transaction().execute(async (trx) => {
        ...
        if (orderId === null)
            throw conflict('Could not allocate an order number. Try again.');
        await trx
            .insertInto('order_details')
            .values(input.items.map((it) => ({
            order_id: orderId,
...
export function validateUpdateOrderInput(body) {
    const b = (body ?? {});
    const out = {};
    if (b.customer_name !== undefined) {
        if (!b.customer_name)
            throw badRequest('Customer name cannot be blank.');
    ...
    for (const key of ['alt_mobile', 'email', 'gst', 'address', 'dues_date', 'show_name_input']) {
        if (b[key] !== undefined)
            out[key] = b[key] == null ? null : String(b[key]);
    }
    if (b.show_name_in_card !== undefined)
        out.show_name_in_card = b.show_name_in_card ? 1 : 0;
    if (b.show_image_in_card !== undefined)
        out.show_image_in_card = b.show_image_in_card ? 1 : 0;
    ...
    return db.transaction().execute(async (trx) => {
        ...
        if (existing.length) {
            const counts = await trx
                ...
                .where('order_detail_id', 'in', existing.map((e) => String(e.id)))
                .groupBy('order_detail_id')
                .execute();
            for (const c of counts)
                certified.set(Number(c.order_detail_id), Number(c.n));
        }
        ...
        for (const row of existing) {
            ...
            if (issued > 0) {
                throw conflict(`Item ${row.id} cannot be removed: ${issued} certificate${issued === 1 ? ' has' : 's have'} already been issued against it.`);
            }
        ...
        for (const item of input.items) {
            if (item.id) {
                ...
                if (item.qty < issued) {
                    throw conflict(`Item ${item.id} cannot drop to ${item.qty}: ${issued} certificates have already been issued.`);
                }
                await trx
