 ...
 * changed hands.
 */
const TEMPLATE = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '../templates/order-document.ejs');
...
export async function orderDocumentHtml(orderId, kind) {
    ...
    if (kind === 'invoice') {
        ...
        for (const c of quote.certificates)
            detailOf.set(c.report_id, c.line_total);
...
export async function orderDocumentPdf(orderId, kind) {
