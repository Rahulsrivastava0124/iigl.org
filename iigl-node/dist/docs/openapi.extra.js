...
function crud(tag, noun, collection, createProps, createRequired, patchProps, extras = {}) {
    return {
        [collection]: {
            post: {
                tags: [tag],
...
export const extraPaths = {
    ...
    ...crud('Catalogue admin', 'category', '/api/admin/categories', {
        ...
        banner: str,
        icon: str,
    }, ['name', 'unit'], { name: { type: 'string' }, unit: int, description: str, short_description: str, banner: str, icon: str }),
    ...crud('Catalogue admin', 'subcategory', '/api/admin/subcategories', { name: { type: 'string' }, category_id: int, description: str, banner: str, icon: str }, ['name', 'category_id'], { name: { type: 'string' }, category_id: int, description: str, banner: str, icon: str }),
    ...crud('Catalogue admin', 'attribute', '/api/admin/attributes', {
        ...
        '/api/admin/attributes/{id}': {
            delete: {
                ...
                description: 'A soft delete. 22,103 certificates hold attribute ids inside reports.description, and removing the row would render those cards with a blank field.',
                parameters: [idParam],
                responses: { 200: ok('Retired.'), 404: err('Attribute not found.'), ...guarded },
            },
        },
    }),
    ...crud('Catalogue admin', 'attribute value', '/api/admin/attribute-values', { attr_id: int, value_name: { type: 'string' }, description: str, icon: str }, ['attr_id', 'value_name'], { attr_id: int, value_name: { type: 'string' }, description: str, icon: str }, {
    ...
    ...crud('Catalogue admin', 'price band', '/api/admin/prices', {
        category_id: int,
        ...
        '/api/admin/prices/{id}': {
            delete: {
                ...
                description: 'A real delete. An order keeps the total it was billed rather than a reference to the band that produced it, so nothing points here.',
                parameters: [idParam],
    ...
    '/api/roles': {
        get: {
            ...
            description: 'Head office sees every role. A laboratory sees the shared roles and its own, not another laboratory\'s. Each row carries how many people hold it.',
            responses: { 200: ok('Roles, system ones first.'), ...guarded },
        },
        post: {
            tags: ['Permissions'],
            summary: 'Create a role',
            description: 'Head office creates a shared role; a laboratory creates one of its own, which only its staff can be given. The name must be unique among the roles the creator can see — two laboratories may both have a "Front desk".',
    ...
    '/api/roles/actions': {
        get: {
            ...
            description: 'From `permission_actions`. `enforced` is false for a name the API does not yet check — it appears on the permission screens but grants nothing until a check is written against it.',
            responses: { 200: ok('The permission list.'), ...guarded },
        },
        post: {
            tags: ['Permissions'],
            summary: 'Add a permission to the list',
            description: 'Head office only. The name is lower-cased and underscored. Adding one puts it on every permission screen; it is a label until the API reads it.',
    ...
    '/api/roles/{id}/users': {
        get: {
            ...
            description: 'The accounts on it, by name. Scoped like the role itself: head office sees any role, a laboratory only the shared ones and its own.',
            parameters: [idParam],
    ...
    '/api/roles/{id}/permissions': {
        ...
        put: {
            ...
            parameters: [idParam],
            requestBody: body({ action_type: { type: 'string' }, view: bool, create: bool, update: bool, delete: bool }, ['action_type']),
            responses: {
                200: ok('Saved.'),
    ...
    '/api/users/{id}/permissions': {
        ...
        put: {
            ...
            description: 'An individual grant **replaces** the role\'s answer for that action rather than adding to it, so it can take away as well as give — all four flags off means "not this, whatever the role says". It is also how a user with no role gets anything at all.',
            parameters: [idParam],
            requestBody: body({ action_type: { type: 'string' }, view: bool, create: bool, update: bool, delete: bool }, ['action_type']),
            responses: {
                200: ok('Saved.'),
    ...
    '/api/students/enquiries': {
        get: {
            ...
            description: 'The first stage: somebody asking about a course. Distinct from /api/enquiries, which is the general enquiry book — a course enquiry carries the course they are interested in and converts into a registration.',
            parameters: [
    ...
    '/api/students/enquiries/{id}': {
        patch: {
            ...
            description: 'Every field but the status, which walks new → contacted → interested and then either not_interested or — through the convert endpoint only — converted.',
            parameters: [idParam],
            requestBody: body({
                name: { type: 'string' },
                mobile: { type: 'string' },
    ...
    '/api/students/enquiries/{id}/convert': {
        post: {
            ...
            description: 'One transaction: the student is registered, given a registration number, and the enquiry is marked converted and pointed at them. A converted enquiry with no student behind it would drop off the follow-up list while nobody was registered.',
            parameters: [idParam],
            requestBody: body({
    ...
    '/api/students': {
        ...
        post: {
            ...
            description: 'The registration number is issued here — IIGL-YYYY-NNNN, counted within the year and taken in the same transaction as the row, so two registrations at once cannot share one.',
            requestBody: body({
                name: { type: 'string' },
                father_name: str,
                dob: str,
    ...
    '/api/courses/{id}': {
        patch: {
            ...
            description: 'Changing the catalogue fee does not touch anybody\'s enrolment: an enrolment holds what that student was billed.',
            parameters: [idParam],
            requestBody: body({
                name: { type: 'string' },
                code: str,
    ...
    '/api/courses/enrolments': {
        ...
        post: {
            ...
            description: 'The fee is copied from the catalogue rather than read through it, so a price change next year cannot restate what this student was billed. Enrolling also makes the registration active.',
            requestBody: body({
                student_id: int,
                course_id: int,
                batch: str,
    ...
    '/api/courses/enrolments/{id}': {
        patch: {
            ...
            description: 'Changing the fee recomputes the discount, because a discount is a rule rather than a number. Marking it completed stamps the completion date.',
            parameters: [idParam],
            requestBody: body({
                batch: str,
                start_date: str,
    ...
    '/api/student-certificates': {
        get: {
            ...
            description: 'Not the gemstone certificates in /api/reports. These are numbered IIGL-C-YYYY-NNNN so the two cannot be mistaken for one another across a desk.',
            parameters: [
                { name: 'page', in: 'query', schema: { type: 'integer' } },
                { name: 'per_page', in: 'query', schema: { type: 'integer', maximum: 200 } },
                { name: 'student_id', in: 'query', schema: { type: 'integer' } },
        ...
        post: {
            ...
            description: 'Against a completed enrolment, not against a student: two courses earn two certificates. Refused while the course is unfinished.',
            requestBody: body({ student_course_id: int, issued_on: str, grade: str, remark: str, file: str }, ['student_course_id']),
            responses: {
    ...
    '/api/enquiries': {
        get: {
            ...
            description: 'Administrators only. Ordered so anything still open outranks anything closed — the list is a queue of work, not an archive.',
            parameters: [
                { name: 'page', in: 'query', schema: { type: 'integer' } },
                { name: 'per_page', in: 'query', schema: { type: 'integer', maximum: 200 } },
                {
    ...
    '/api/customers/registered': {
        get: {
            ...
            description: 'There is no customer table. A customer is whoever has placed an order, so this groups orders by mobile number. "Registered" means a GST number was given, which is the only distinction the data draws. Scoped the same way the order list is, including the product_collection narrowing for staff.',
            parameters: [
                { name: 'page', in: 'query', schema: { type: 'integer' } },
                { name: 'per_page', in: 'query', schema: { type: 'integer', maximum: 200 } },
            ],
    ...
    '/api/customers/verifiers': {
        get: {
            ...
            description: 'From reportsearches, which the public verification form writes one row to per lookup. Grouped by mobile number.',
            parameters: [
                { name: 'page', in: 'query', schema: { type: 'integer' } },
                { name: 'per_page', in: 'query', schema: { type: 'integer', maximum: 200 } },
            ],
    ...
    '/api/attendance': {
        get: {
            ...
            description: 'Your own by default. A laboratory or administrator can read one of their people with emp_id. `from` and `to` are inclusive dates, either given alone: a calendar asks for a month that way, because paging newest-first can split one across two pages and a calendar that pages to fill itself in draws holes.',
            parameters: [
                { name: 'page', in: 'query', schema: { type: 'integer' } },
                { name: 'per_page', in: 'query', schema: { type: 'integer', maximum: 200 } },
    ...
    // -------------------------------------------------------------- content
    ...crud('Content', 'article', '/api/content/blogs', {
    ...
    ...crud('Content', 'branch page', '/api/content/branches', { city: { type: 'string' }, pageURL: { type: 'string' }, h1: str, content: str, img: str, title: str, description: str, keywords: str }, ['city'], { city: str, pageURL: { type: 'string' }, h1: str, content: str, img: str, title: str, description: str, keywords: str }),
    ...crud('Content', 'certificate type', '/api/content/report-types', { name: { type: 'string' }, short_description: str, description: str, banner: str, icon: str, meta_title: str, meta_description: str, meta_keywords: str }, ['name'], { name: { type: 'string' }, short_description: str, description: str, banner: str, icon: str, meta_title: str, meta_description: str, meta_keywords: str }),
    ...crud('Content', 'banner', '/api/content/banners', { path: { type: 'string', description: 'An uploaded path from POST /api/uploads/banner.' }, img_type: { type: 'string' }, name: str, url: str, status: bool }, ['path', 'img_type'], { path: { type: 'string' }, img_type: { type: 'string' }, name: str, url: str, status: bool }, {
        '/api/content/banners': {
            get: {
                tags: ['Content'],
    ...
    '/api/users/me': {
        patch: {
            ...
            description: 'Excludes mobile, role, active flag and commission: the first is the sign-in identifier and the rest decide what the account may do.',
            requestBody: body({
    ...
    '/api/users/{id}/employment': {
        ...
        patch: {
            ...
            description: 'The salary and the joining date live on the employment, not on the account, so PATCH /api/users/{id} cannot reach them. Moving somebody to another employer is not this — that is ending one employment and starting another, which keeps the history.',
            parameters: [idParam],
            requestBody: body({
                salary: { type: 'number' },
    ...
    '/api/cards/order/{kind}/{id}': {
        get: {
            ...
            description: 'A receipt lists what was taken in and carries no prices, because nothing is priced until the certificates exist. An invoice adds the money, taken from the pricing service rather than the stored columns so it can never disagree with the settle screen.',
            parameters: [
    ...
    '/api/coupons': {
        get: {
            ...
            description: 'Head office only, like every course route. Each row names its course (null is any course) and carries `spent` — the usage limit is reached — and `expired`, so a list never shows a coupon as usable when nobody can use it.',
            parameters: [
        ...
        post: {
            ...
            description: 'A coupon is money off a **course fee** and nothing else. The code is upper-cased and stripped of spaces: it is read off a printout and typed back in. `course_id` ties it to one course; null is any course we run.',
            requestBody: body({
    ...
    '/api/coupons/{id}': {
        ...
        delete: {
            ...
            description: 'Only one that has never been spent. A coupon with redemptions is switched off instead \u2014 deleting it would take the record of money already taken off a student\u2019s fee with it.',
            parameters: [idParam],
            responses: {
                200: ok('Deleted.'),
    ...
    '/api/coupons/{id}/redemptions': {
        get: {
            ...
            description: 'One enrolment per row, newest first: the student, the course, the fee before, the discount, what was left, and who applied it. The row keeps the code as it stood at the time \u2014 a coupon can be renamed, what a student was charged cannot.',
            parameters: [
                idParam,
                { name: 'page', in: 'query', schema: int },
                { name: 'per_page', in: 'query', schema: int },
