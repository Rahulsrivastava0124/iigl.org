...
const tagNote = new Map((openApiDocument.tags ?? []).map((t) => [t.name, t.description ?? '']));
rows.sort((a, b) => (tagOrder.get(a.tag) ?? 99) - (tagOrder.get(b.tag) ?? 99) ||
    a.path.localeCompare(b.path) ||
    METHOD_ORDER.indexOf(a.method) - METHOD_ORDER.indexOf(b.method));
const isPublic = (op) => Array.isArray(op.security) && op.security.length === 0;
...
function bodyFields(op) {
    ...
    if (names.length > 8)
        shown.push(`+${names.length - 8} more`);
    return shown.join(', ');
}
...
w('# IIGL API');
w();
w(`${rows.length} endpoints. Generated from the OpenAPI document by \`npm run docs\` — do not edit by hand.`);
w();
...
for (const row of rows) {
    ...
    w(`| ${row.method} | \`${row.path}\` | ${isPublic(row.op) ? 'public' : 'session'} | ` +
...
w();
w(`${rows.length} endpoints: ${publicCount} public, ${rows.length - publicCount} requiring a session.`);
w();
