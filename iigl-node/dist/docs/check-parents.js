...
async function main() {
    ...
    const rows = await db
        ...
        .leftJoin('employements as e', (join) => join.onRef('e.user_id', '=', 'u.id').on('e.is_working', '=', '1'))
    ...
    console.log(`  ${pad('who', width)}  ${pad('users', 12)}  ${pad('employements', 12)}  what it means`);
    for (const d of drifted) {
        ...
        const meaning = d.on_employment !== null && !d.employment_resolves
        ...
        console.log(`  ${pad(d.fullname, width)}  ${pad(d.on_user ?? '—', 12)}  ${pad(d.on_employment ?? '—', 12)}  ${meaning}`);
    }
