...
for (const order of priced) {
    ...
    if (quote.certificates.length === 0)
        noCertificates++;
...
if (mismatches.length) {
    ...
    for (const m of mismatches.slice(0, 15)) {
        console.log(`    ${String(m.id).padEnd(8)} ${String(m.stored).padStart(6)} ${String(m.computed).padStart(10)} ${String(m.diff).padStart(9)} ${String(m.unpriced).padStart(9)}`);
    }
}
await db.destroy();
